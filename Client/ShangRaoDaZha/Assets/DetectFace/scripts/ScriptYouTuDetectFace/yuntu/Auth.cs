...
public class Auth
{
    ...
    {
        return string.Format("u={0}&a={1}&k={2}&e={3}&t={4}&r={5}&f={6}", uid, appid, secretid, etime, stime,
            new Random()
                .Next(0, 1000000000), "");
    }
    ...
    public static string appSign(string expired, string userid)
    {
        ...
        string plainText = SetOrignal(Conf.Instance().USER_ID, Conf.Instance().APPID, Conf.Instance().SECRET_ID, time,
            expired);

        byte[] signByteArrary = Utility.JoinByteArr(HmacSha1Sign(plainText, Conf.Instance().SECRET_KEY),
            Utility.StrToByteArr(plainText));
