...
public class DDZJinBi : UIBase<DDZJinBi>
{
    ...
    public UIButton InviteFriend;//邀请朋友(换桌)
    ...
    void ShareInvite()
    {
      ...
      //  ManagerScene.Instance.LoadScene(SceneType.Main);
        ...
        //    case 0:
        //        // jishu =1111;
        //        AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局" + "  房主支付!", "闲娱狗讨赏");
        //        // RuleLable.text = "局数：" + GameData.m_TableInfo.curGameCount.ToString() + "/" + GameData.m_TableInfo.configRoundIndex.ToString() + "  房主支付";
        //        //  jishu = 11112222;
        //        break;
        //    case 1:
        //        //  jishu = 2222;
        //        AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局" + "  平摊支付!", "闲娱狗讨赏");
    ...
    public void OnShowFriendCard(int pos, uint CardValue)
    {
        if (pos != 0)
        {
          ...
          //  IdAndPlayerDic[pos].transform.FindChild("FriendCard").gameObject.SetActive(true);
            IdAndPlayerDic[pos].transform.Find("FriendCard").GetComponent<Card>().SetValue(CardValue);
        }
        //  if(SelfPos==)
    ...
    public void ReconnectServer()
    {
        ...
        else if (GameData.m_TableInfo.roomState == RoomStatusType.Play)
        {
            ...
            for (int i = 0; i < GameData.m_PlayerInfoList.Count; i++)
            {
                ...
                SetTotalJifen(GameData.m_PlayerInfoList[i].pos, GameData.m_PlayerInfoList[i].Gold);//重置总积分
           ...
           // ShowOperatePanle(GameData.m_TableInfo.waitOutCardPos, false);
    ...
    private void SetPlayerListPos()
    {
        ...
        for (int i = 0; i < GameData.m_PlayerInfoList.Count; i++)
        {
            ...
            SetTotalJifen(GameData.m_PlayerInfoList[i].pos, GameData.m_PlayerInfoList[i].Gold);//设置积分
    ...
    /// </summary>
    /// <param name="pos"></param>
    public void SetTotalJifen(int pos, long Score)
    {
    ...
    public void onPlayerEnter(PlayerInfo info)
    {
        ...
        SetTotalJifen(info.pos, info.Gold);//设置积分
    ...
    private IEnumerator AsynDealCard()
    {
        for (int i = 0; i < LocalCardsObjList.Count; i++)
        {
            // LocalCardsObjList[i].transform.localPosition = new Vector3(-440 + 32 * i, 0, 0);
   ...
   // public List<uint> SelfCardList = new List<uint>();//自己的手牌
    ...
    public void CardSort()
    {
        ...
        if (LocalCardsObjList.Count >= 23)//25张的排序
        {
        ...
        else if (LocalCardsObjList.Count < 23 && LocalCardsObjList.Count > 17)
        {
            if (LocalCardsObjList.Count % 2 == 1)//基数张
            {
                for (int i = 0; i < LocalCardsObjList.Count; i++)
                {
                    LocalCardsObjList[i].transform.localPosition = new Vector3(((LocalCardsObjList.Count - 1) / 2) * 50 - 40 * i, CardPoint.transform.position.y, 0);
            ...
            else //偶数张
            {
                for (int i = 0; i < LocalCardsObjList.Count; i++)
                {
                    LocalCardsObjList[i].transform.localPosition = new Vector3(((LocalCardsObjList.Count - 1) / 2) * 50 - 40 * i - 50, CardPoint.transform.position.y, 0);
                    if (i != 0)
                    {
                        LocalCardsObjList[i].transform.GetComponent<UISprite>().depth = LocalCardsObjList[i - 1].transform.GetComponent<UISprite>().depth - 1;
                    }
        ...
        else if (LocalCardsObjList.Count <= 17 && LocalCardsObjList.Count > 12)
        {
            if (LocalCardsObjList.Count % 2 == 1)//基数张
            {
                for (int i = 0; i < LocalCardsObjList.Count; i++)
                {
                    LocalCardsObjList[i].transform.localPosition = new Vector3(((LocalCardsObjList.Count - 1) / 2) * 40 - 45 * i, CardPoint.transform.position.y, 0);
                    if (i != 0)
                    {
                        LocalCardsObjList[i].transform.GetComponent<UISprite>().depth = LocalCardsObjList[i - 1].transform.GetComponent<UISprite>().depth - 1;
                    }
            ...
            else //偶数张
            {
                for (int i = 0; i < LocalCardsObjList.Count; i++)
                {
                    LocalCardsObjList[i].transform.localPosition = new Vector3(((LocalCardsObjList.Count - 1) / 2) * 40 - 45 * i + 50, CardPoint.transform.position.y, 0);
                    if (i != 0)
                    {
                        LocalCardsObjList[i].transform.GetComponent<UISprite>().depth = LocalCardsObjList[i - 1].transform.GetComponent<UISprite>().depth - 1;
                    }
        ...
        else if (LocalCardsObjList.Count <= 12 && LocalCardsObjList.Count > 5)
        {
            if (LocalCardsObjList.Count % 2 == 1)//基数张
            {
                for (int i = 0; i < LocalCardsObjList.Count; i++)
                {
                    LocalCardsObjList[i].transform.localPosition = new Vector3(((LocalCardsObjList.Count - 1) / 2) * 55 - 50 * i, CardPoint.transform.position.y, 0);
                    if (i != 0)
                    {
                        LocalCardsObjList[i].transform.GetComponent<UISprite>().depth = LocalCardsObjList[i - 1].transform.GetComponent<UISprite>().depth - 1;
                    }
        ...
        else if (LocalCardsObjList.Count <= 5 && LocalCardsObjList.Count > 0)
        {
            if (LocalCardsObjList.Count % 2 == 1)//基数张
            {
                for (int i = 0; i < LocalCardsObjList.Count; i++)
        ...
        //try
        //{
        //    if (CardSortType == SortType.NumSort)//如果需要用数量排序
        //    {
        //        List <CardData> a= CardDataNumSort(CardDataList);
        //    }
        //}
