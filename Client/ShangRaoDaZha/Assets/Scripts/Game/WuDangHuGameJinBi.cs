...
public class WuDangHuGameJinBi : UIBase<WuDangHuGameJinBi>
{
    ...
    void OnClickBtn(GameObject go)
    {
        ...
        switch (go.name)
        {
            case "btnInvite":
                ...
                //        {
                //            AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局" + "  房主支付!带奖码!", "闲娱狗无挡胡");
                //        }
                //        else
                //        {
                //            AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局" + "  房主支付!不带奖码!", "闲娱狗无挡胡");
                //        }
                //        break;
                //    case 1:
                //        if (GameData.m_TableInfo.IsJIangMa)
                //        {
                //            AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局" + "  平摊支付!带奖码!", "闲娱狗无挡胡");
                //        }
                //        else
                //        {
                //            AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局" + "  平摊支付!不带奖码!", "闲娱狗无挡胡");
                //        }
    ...
    void ShowLocalOperateState(byte pos, uint card, bool isMoPai = true,bool IsTianHu=false)
    {
        ...
        if (pos == LocalPos && isMoPai)//自己摸上了的牌
        {
            ...
            if (IsTianHu)
            {
                isHu = myXYHelper.Instance.WDHCheckHu(operateList, info.localCardList, card, true);//是否杠开
            }
            else
            {
    ...
    void CreateOtherPlayerGetInCard(Transform TargetParent)
    {
        ...
        if (TargetParent.Find((TargetParent.childCount - 1).ToString()) != null)
        {
            ...
            {
                TargetParent.Find((activeCount-1).ToString()).gameObject.SetActive(true);
