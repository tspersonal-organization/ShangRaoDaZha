...
public class WuDangHuGame : UIBase<WuDangHuGame>
{
    ...
    public int OutInterval = 15;//出牌倒计时的间隔
    ...
    public GameObject GangPaiPanel;//杠提示
    ...
    void OnClickBtn(GameObject go)
    {
        ...
        {
            case "btnInvite":
                // string tmp = roomDesc.Replace("\n", "  ");
               // string shareStr = "游戏规则:" + gameRoundCount[GameData.m_TableInfo.configRoundIndex] + "局  ";// + tmp;
               // string shareStr = "游戏规则:" +GameData.m_TableInfo.configRoundIndex + "局  ";// + tmp;
               // AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, shareStr, "【闲娱狗无挡胡】");
                ...
                switch ((int)GameData.m_TableInfo.configPayIndex)
                {
                    ...
                    case 0:
                        ...
                        {
                            AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局!" + (GameData.m_PlayerInfoList.Count).ToString()+"缺" + (4 - GameData.m_PlayerInfoList.Count).ToString() + "!" + "  房主支付!带奖码!", "闲娱狗无挡胡");
                        }
                        else
                        {
                            AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局!" + (GameData.m_PlayerInfoList.Count).ToString() + "缺" + (4 - GameData.m_PlayerInfoList.Count).ToString() + "!" + "  房主支付!不带奖码!", "闲娱狗无挡胡");
                        }
                        break;
                    case 1:
                        if (GameData.m_TableInfo.IsJIangMa)
                        {
                            AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局!" + (GameData.m_PlayerInfoList.Count).ToString() + "缺" + (4 - GameData.m_PlayerInfoList.Count).ToString() + "!" + "  平摊支付!带奖码!", "闲娱狗无挡胡");
                        }
                        else
                        {
                            AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局!" + (GameData.m_PlayerInfoList.Count).ToString() + "缺" + (4 - GameData.m_PlayerInfoList.Count).ToString() + "!" + "  平摊支付!不带奖码!", "闲娱狗无挡胡");
                        }
                       // AuthorizeOrShare.Instance.ShareRoomID(GameData.m_TableInfo.id, "一起来玩闲娱狗吧！" + ((int)GameData.m_TableInfo.configRoundIndex).ToString() + "局" + "  平摊支付!", "闲娱狗无挡胡");
                        break;
                }
    ...
    void InitPlayerData()
    {
        ...
        for (int i = 0; i < GameData.m_PlayerInfoList.Count; i++)
        {
            ...
            GameData.m_PlayerInfoList[i].LVD= GetLVD(info.pos);
    ...
    void RoomPlay()
    {
        ...
        else
        {
            ...
            if (GameData.m_TableInfo.isInCardInfo)
            {
                ...
                {
                    SelfHaveGone = GameDataFunc.GetPlayerInfo(LocalPos).IsGangKai;//杠开短线回来
                    ShowLocalOperateState(InCardPos, InCardNumber, true, GameDataFunc.GetPlayerInfo(LocalPos).IsFirstChuPai);
                }
    ...
    public Transform JiangMaParent;
    public List<GameObject> JiangMaObjList = new List<GameObject>();
    public List<uint> JiangMaList = new List<uint>();
    /// <summary>
    ///  广播奖码信息
    /// </summary>
    /// <param name="JiangMaList">奖码牌</param>
    public void OnJiangMa(List<uint> JiangMaList)
    {
        JiangMaObjList = new List<GameObject>();
        this.JiangMaList = JiangMaList;
        JiangMaParent.gameObject.SetActive(true);
     
        //for (int i = 0; i < JiangMaList.Count; i++)
        //{
        //    if (pbCardImage == null) pbCardImage = Resources.Load<GameObject>("Item/CardImage");

        //    GameObject card = Instantiate<GameObject>(pbCardImage);
        //    card.transform.parent = JiangMaParent;
        //    card.transform.localScale = Vector3.one;
        //    card.transform.localPosition = new Vector3(-120+80*i,0,0);
        //    card.name = JiangMaList[i].ToString();
        //   // card.AddComponent<BoxCollider2D>().size = new Vector3(75, 122, 0);
        //    // card.GetComponent<UISprite>().spriteName = (GameData.m_TableInfo.curGameCount % 2 == 0) ? "card_local_max_" + info.localCardList[j].ToString() : "cardblue_local_max_" + info.localCardList[j].ToString();
        //    card.GetComponent<UISprite>().spriteName = "card_local_max_" + JiangMaList[i].ToString();
        //    JiangMaObjList.Add(card);
        //}
    ...
    /// 重置奖码物体
    /// </summary>
    public void ResetJiangMaObj()
    {
        if (JiangMaObjList != null)
        {
            for (int i = 0; i < JiangMaObjList.Count; i++)
            {
                Destroy(JiangMaObjList[i]);
            }
        }
        JiangMaObjList = new List<GameObject>();
   ...
   /// </summary>
    ...
    #region  选分相关  不需要
    public void onWaitPlayerChooseScore()
    {
        if (GameData.m_TableInfo.configDaZiIndex == 0) return;
        TranFPBtn.gameObject.SetActive(true);
        for (int i = 0; i < 2 + GameData.m_TableInfo.configDaZiIndex; i++)
        {
            TranFPBtn.Find("btnFP" + i).gameObject.SetActive(true);
        }
        TranFPBtn.GetComponent<UIGrid>().enabled = true;
    }
    public void onPlayerChooseScore(byte pos, uint chooseScore)
    {
        if (pos == LocalPos) TranFPBtn.gameObject.SetActive(false);
        if (chooseScore > 0)
        {
            GameObject obj = PlayerObjList[(int)GetLVD(pos)];
            UISprite sp = obj.transform.Find("fpScore").GetComponent<UISprite>();
            sp.gameObject.SetActive(true);
            sp.spriteName = "UI_game_icon_bet_" + chooseScore;
        }
        GameDataFunc.GetPlayerInfo(pos).fangPaoScore = chooseScore;
    }
    ...
    IEnumerator AsynDealCard()
    {
        ...
        int tmpGivenCards = 0;
        if (pbCardImage == null) pbCardImage = Resources.Load<GameObject>("Item/CardImage");
        for (int i = 0; i < inCardCount; i++)//发牌 4个4个的发
        {
       # region 发牌
            for (int j = 0; j < cardCount; j++)//本地牌
            {
                GameObject card = Instantiate<GameObject>(pbCardImage);
                card.transform.parent = LocalParent;
                card.transform.localScale = Vector3.one;
        ...
        if (InCardPos == LocalPos) ShowLocalOperateState(InCardPos, InCardNumber,true,true);
    ...
    void ShowLocalOperateState(byte pos, uint card, bool isMoPai = true,bool FirstIndex=false)
    {
        ...
        if (pos == LocalPos && isMoPai)//自己摸上了的牌
        {
            ...
            if (FirstIndex)//是否为天胡
            {
                isHu = myXYHelper.Instance.WDHCheckHu(operateList, info.localCardList, card, true);//是否为天胡
            }
            else
            {
                isHu = myXYHelper.Instance.WDHCheckHu(operateList, info.localCardList, card, SelfHaveGone);//是否杠开
            }
    ...
    public void CheckTing()
    {
        ...
        try
        {
            TingDic = myXYHelper.Instance.WDHCheckTing(OutCard, operateList, HoldCardList);
            if (TingDic == null)
            {
                TingDic = new Dictionary<uint, List<Dictionary<uint, HuCardInfo>>>();
            }
...
void CreateOtherPlayerGetInCard(Transform TargetParent)
{
    ...
    if (TargetParent.Find((TargetParent.childCount - 1).ToString()) != null)
    {
        ...
        {
            TargetParent.Find(activeCount.ToString()).gameObject.SetActive(true);
            tmp.localPosition = TargetParent.Find((activeCount).ToString()).localPosition + new Vector3(localCardWidth * 2f, 0, 0);
        }
        else
        {
            tmp.gameObject.SetActive(true);
            tmp.localPosition = TargetParent.Find((activeCount - 1).ToString()).localPosition + new Vector3(localCardWidth * 2f, 0, 0);
        }
    }
}
/// <summary>
...
void InitCardsOnDesk()
{
        ...
        {
            int sum = GameData.Dice1 + GameData.Dice2;//两者之和，并且以庄家为起点
            int sp = sum;
            if (GameData.m_TableInfo.configPlayerIndex == 2)
            {
                if (GameData.m_TableInfo.makerPos == 2)
                {
                    sum = sum + 2;
                }
                else
            ...
            {
                sum += GameData.m_TableInfo.makerPos - 1;
            }
            ...
            int index = 0;
            switch (GetLVD((byte)(sum), true))
            {
                case LocalViewDirection.LEFT:
                    for (int i = sp; i < leftDeskStartStack; i++)
                    {
...
void SetCardsOnDesk(int RemainedCards)
{
    ...
    int startDisableCount = TotalCardsCount - RemainedCards - totalGangCount;
    for (int i = 0; i < startDisableCount; i++)
    {
        AllCardsOnDesk[i].SetActive(false);
    }
    for (int i = 0; i < totalGangCount; i++)
    {
...
IEnumerator AsynPengGang(ulong playerGuid, CardOperateType opType, uint card, byte outPos, bool isAnGang, bool isNormal = true)
{
        ...
        if (isBuGang)
        {
            ...
            //    //}
            //    //else
            //  isHu = myXYHelper.Instance.WDHCheckHu(operateList, tempList, card, isBuGang);
            //    int index = 0;
            //    if (isHu)
            //    {
            //        btnGuo.SetActive(true);
   ...
   /// <param name="type"></param>
   /// <returns></returns>
 IEnumerator AsynCreateEffect(LocalViewDirection lvd, CardOperateType type)
{
